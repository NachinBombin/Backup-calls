...
-- Fires cball_explode + 2x ManhackSparks at the muzzle position
-- scale matches weapon tier: 1 = GAU, 2 = 40mm, 3 = 105mm
function ENT:SpawnWeaponMuzzleFX(effectName, scale)
    ...
    -- 2 manhack spark bursts at the same muzzle position, same scale
    for _ = 1, 2 do
        local sp = EffectData()
...
function ENT:FireSingleGAUBullet(bulletIndex)
    ...
    gred.CreateBullet(self, muzzlePos, dir:Angle(), self.GAU_Caliber, { self }, nil, false, self.GAU_TracerColor, self.GAU_DamageMul, self.GAU_RadiusMul, false)
end
...
function ENT:Update40mm(ct)
    ...
    local muzzlePos = self:GetMuzzlePos()
    local aimTarget = self:GetTargetGroundPos() + Vector(math.Rand(-self.GUN40_Scatter, self.GUN40_Scatter), math.Rand(-self.GUN40_Scatter, self.GUN40_Scatter), 0)
...
function ENT:Update105mm(ct)
    ...
    local muzzlePos = self:GetMuzzlePos()
    local aimTarget = self:GetTargetGroundPos() + Vector(math.Rand(-self.GUN105_Scatter, self.GUN105_Scatter), math.Rand(-self.GUN105_Scatter, self.GUN105_Scatter), 0)
